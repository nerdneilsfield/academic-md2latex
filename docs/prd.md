...
- **academic-md2latex**：项目/仓库名
- **md-mid**：本项目定义的“学术写作中间格式”名称（同时也作为 CLI 工具名，便于记忆）
...
md-mid 主要覆盖两类写作流，对应两种常见的 LaTeX 工程组织方式：
...
1. 按章节/小节拆分写作：每个章节一个 md-mid 文件（建议命名为 `sec1.mid.md` 之类，便于区分源文件与生成物）。
...
4. 编译主工程：用 `latexmk` / `xelatex` 编译 `main.tex`；需要修改内容时，只编辑 md-mid，再重新生成对应章节 `.tex`。

#### B. 独立写作（单文件输出可编译 LaTeX）

**适用场景**：你希望一份 md-mid 直接生成完整、可编译的 `paper.tex`（standalone）。
...
- 若源文件包含 YAML front matter（`--- ... ---`）且启用了相关解析插件，则 front matter 会被视为语义内容块并结束头部区域（建议将 md-mid 文档级指令放在 front matter 之前；或仅使用本项目的 HTML 注释指令系统）
...
链接文本（`Wang et al.`、`1-3`）是**显示文本**，仅用于 md-mid 源文件的可读性和 Rich Markdown 输出，LaTeX 输出时丢弃（由 `\cite` + bibstyle 自动生成格式）。显示文本允许为空（如 `[](cite:wang2024)`）。
...
md-mid chapter3.mid.md -t latex --mode body -o chapter3.tex

# 生成 Rich Markdown 预览
md-mid paper.mid.md -t markdown --bib refs.bib -o paper.rendered.md

# 批量转换
md-mid chapters/*.mid.md -t latex --mode body -o build/
...
**目标**：建立可运行的转换主干（md-mid → EAST → LaTeX），覆盖最常见正文结构，并输出可定位的诊断信息。
